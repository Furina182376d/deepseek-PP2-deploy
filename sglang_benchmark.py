...
# The server's built-in warmup posts one request to itself with a hard 600 s
# timeout and kills the process tree when it expires. Benchmark traffic queues
# ahead of it (FCFS), so a slow cold start takes the whole server down. This
# benchmark's own first pass is discarded anyway, so it covers the same cold
# path without the self-kill timer.
